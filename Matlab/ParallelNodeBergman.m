...
miniBatchSize = 3;
plotFrequency = 20;
weights = 1; %./ (max(xTrain(1:3,:), [], 2)-min(xTrain(1:3,:), [], 2) + 0.01);
...
while iteration < numIter && ~monitor.Stop
    ...
    if mod(iteration, plotFrequency) == 0 || iteration == 1
        ...
        y = dlode45(@(t,y,p) odeModel(t,y,p,U(t),D(t)), timesteps, X0, neuralOdeParameters, DataFormat="CB");
...
function X = model(tspan, X0, neuralOdeParameters,U,D)
% Solve ODE using dlode45
% d=dlarray(zeros(1,550))
% u=dlarray(zeros(1,550))
X = dlode45(@(t,y,p) odeModel(t,y,p,U(t),D(t)), tspan, X0, neuralOdeParameters,DataFormat="CB");
X = cat(3,X0,X);
end

function y = odeModel(t, y, theta,U,D)
...
% Compute Predictions
miniBatchSize = size(targets, 2); % Number of mini-batches
X = dlarray(zeros([size(targets, 1), miniBatchSize, size(targets, 3)], 'like', targets)); % Preallocate
global U D;

for btc = 1:miniBatchSize
    % Solve the Neural ODE for each mini-batch
    Xb = model(tspan(:, btc), X0(:, btc), neuralOdeParameters,U,D);
    X(:, btc, :) = Xb; % Store results
end
...
function neuralOdeParameters = initializeNeuralOdeParameters(stateSize, hiddenSize, externalInputSize, layerNum)
